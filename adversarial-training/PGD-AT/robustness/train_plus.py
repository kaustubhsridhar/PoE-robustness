...
def train_model(args, model, loaders, *, checkpoint=None, dp_device_ids=None,
    ...
    for epoch in range(start_epoch, args.epochs):
        # train for one epoch
        #train_prec1, train_loss = _model_loop(args, 'train', train_loader, model, opt, epoch, args.adv_train, writer)
        train_prec1, train_loss, gradient_vector, params_vector = _model_loop(args, 'train', train_loader, model, opt, epoch, args.adv_train, writer) # New: gradient_vector, params_vector
        ...
        if last_epoch:
            save_object(Lipschitz, os.path.join(args.out_dir,'lipschitz.pkl'))
    ...
    *Internal function* (refer to the train_model and eval_model functions for
    ...
    for i, (inp, target) in iterator:
       # measure data loading time
        ...
        final_inp.requires_grad = True # New: 
        final_inp.retain_grad() # New: 
        same_output = model.forward(final_inp, make_adv=False, with_image=False) # New: 

        #loss = train_criterion(output, target)
        loss = train_criterion(same_output, target) # New: 
        ...
        if is_train:
            ...
            final_inp_grad = final_inp.grad.detach().clone()
            batch_gradients = final_inp_grad.reshape(final_inp.size(0), -1) # collapses [64, 3, 32, 32] to [64, 3*32*32]
            avg_batch_gradients = ch.sum(batch_gradients, dim=0) / final_inp.size(0) # averages rows to give [3*32*32] tensor # Set: keepdim=True to get [1, 3*32*32] tensor
            grads.update(avg_batch_gradients, final_inp.size(0)) # Note: avg_batch_gradient given. Reason: see self.sum in AverageMeter()
    ...
    params = ch.cat(params)

    if is_train:
        return top1.avg, losses.avg, grads.avg.cpu().numpy(), params.cpu().numpy() # New: grads and params also returned
    else:
