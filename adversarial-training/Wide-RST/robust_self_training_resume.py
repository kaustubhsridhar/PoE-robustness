...
# Model config
parser.add_argument('--model', '-m', default='wrn-34-15', type=str,
                    help='Name of the model (see utils.get_model)') # previously, default='wrn-28-10' # for wide paper, we use wrn-34-15
...
parser.add_argument('--epochs', type=int, default=200, metavar='N',
                    help='Number of epochs to train. '
                         ...
                         'configurations.')
parser.add_argument('--resume-from', type=int, default=151,
...
parser.add_argument('--beta', default=21.0, type=float,
                    help='stability regularization, i.e., 1/lambda in TRADES') # for wide paper, we use beta = lamda (in wide paper) = 1/lamda (in TRADES paper) = 21 for RST improved version.
...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    return train_metrics, grads.avg.cpu().numpy(), params.cpu().numpy() # New: grads and params also returned
...
def main():
    ...
    # TODO: Load model
    model_path = os.path.join(model_dir,'checkpoint-epoch{}.pt'.format(args.resume_from-1))
    ...
    opt_path = os.path.join(model_dir,'opt-checkpoint_epoch{}.tar'.format(args.resume_from-1))
    opt_state_dict = torch.load(opt_path)
    optimizer.load_state_dict(opt_state_dict)
    

    Lipschitz = Weibull_Fitter(200, args.epochs) # New: 

    for epoch in range(args.resume_from, args.epochs + 1):
        # adjust learning rate for SGD
        lr = adjust_learning_rate(optimizer, epoch)
        logger.info('Setting learning rate to %g' % lr)
        # adversarial training
        train_data, gradient_vector, params_vector = train(args, model, device, train_loader, optimizer, epoch) # New: gradient_vector, params_vector arguments
